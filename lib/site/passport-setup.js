...
(function() {
  ...
  module.exports = function(expressApp, identityStore, config, baseUrl) {
    ...
    passport.use(new PassportLocalStrategy(function(username, password, done) {
      return identityStore.sessions.signInLocal(username, password, function(err, user) {
        if (err) {
          return done(err);
        }
        return done(null, user);
    ...
    return passport.use(new PassportBearerStrategy(function(token, done) {
      return identityStore.tokenInfos.validate(token, function(err, tokenInfo) {
        trace("Validating Token COMPLETE: " + token);
        if (err) {
          return done(err);
        }
        if (!(tokenInfo && tokenInfo.actor)) {
          return done(null, null);
        }
        return done(null, tokenInfo.actor);
      });
    }));
    /*
