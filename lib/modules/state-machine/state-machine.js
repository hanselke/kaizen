...
(function() {
  ...
  module.exports = StateMachine = (function() {
    ...
    function StateMachine(settings) {
      ...
      this.existsAsExcelField = __bind(this.existsAsExcelField, this);

      this.getExcelFieldFromState = __bind(this.getExcelFieldFromState, this);

      this.getRolesForState = __bind(this.getRolesForState, this);

      this.getReopenStates = __bind(this.getReopenStates, this);

      this.getStatesForRoles = __bind(this.getStatesForRoles, this);
    ...
    */
    ...
    StateMachine.prototype.getStatesForRoles = function(roles) {
      var key, result, role, val, _i, _len, _ref;
      if (roles == null) {
        roles = [];
      }
      result = [];
      ...
      for (key in _ref) {
        val = _ref[key];
        if (val.allowedRoles && val.allowedRoles.length > 0) {
    ...
    StateMachine.prototype.getReopenStates = function() {
      var key, result, val, _ref;
      result = [];
      _ref = this.stateMachineData.states;
      for (key in _ref) {
    ...
    StateMachine.prototype.getRolesForState = function(state) {
      state = this.stateMachineData.states[state];
      if (!state) {
        return [];
      }
      return state.allowedRoles || [];
    };

    StateMachine.prototype.getExcelFieldFromState = function(stateName) {
      var state;
      state = this.getState(stateName);
      if (!state) {
        return null;
      }
      return state.excelField;
    };

    StateMachine.prototype.existsAsExcelField = function(excelFieldName) {
      var key, val, _ref;
      _ref = this.stateMachineData.states;
      for (key in _ref) {
        val = _ref[key];
        if (val.excelField === excelFieldName) {
          return true;
        }
      }
      return false;
