...
(function() {
  ...
  module.exports = RoutesApi = (function() {
    ...
    function RoutesApi(settings, servicesBonita) {
      ...
      this.postAdminTaskForReopen = __bind(this.postAdminTaskForReopen, this);

      this.getAdminTaskForReopen = __bind(this.getAdminTaskForReopen, this);
      ...
      this._addUsernameToAdminTasks = __bind(this._addUsernameToAdminTasks, this);

      this.getAdminMyTasks = __bind(this.getAdminMyTasks, this);
    ...
    RoutesApi.prototype.setupRoutes = function() {
      ...
      this.app.get('/api/admin/tasks/:taskId/reopen', this.getAdminTaskForReopen);
      this.app.post('/api/admin/tasks/:taskId/reopen', this.postAdminTaskForReopen);
      this.app.get('/api/admin/tasks', this.getAdminTasks);
      this.app.get('/api/admin/my-tasks', this.getAdminMyTasks);
    ...
    */
    ...
    RoutesApi.prototype.getAdminTasks = function(req, res, next) {
      ...
      if (filter) {
        return this.dbStore.tasks.allforDay(filter, {
          ...
          offset: 0,
          count: 200,
          select: '_id processDefinitionId state createdAt checkedOutByUserId name taskEnded nextState totalActiveTime totalWaitingTime createdBy'
        }, function(err, result) {
          if (err) {
            return next(err);
          }
          result.items = _.map(result.items, function(x) {
            return x.toObject();
          });
          return _this._addUsernameToAdminTasks(result.items, function(err) {
            return res.json(result);
          });
        });
      } else {
    ...
    RoutesApi.prototype.getAdminMyTasks = function(req, res, next) {
      var filter, userId,
        _this = this;
      if (!req.user) {
        return res.json(401, {});
    ...
    RoutesApi.prototype._addUsernameToAdminTasks = function(tasks, cb) {
      var actorId, idList, task, unresolvedUserIds, _i, _len,
        _this = this;
      if (!this.usernameMap) {
        this.usernameMap = {};
      }
      ...
      for (_i = 0, _len = tasks.length; _i < _len; _i++) {
        ...
        if (task.createdBy && task.createdBy.actorId) {
          ...
          task.username = this.usernameMap[actorId];
          if (!this.usernameMap[actorId]) {
            unresolvedUserIds[actorId] = true;
          }
        }
      }
      if (_.keys(unresolvedUserIds).length === 0) {
        ...
        return this.identityStore.models.User.find({}).where('_id')["in"](idList).select('_id username').exec(function(err, items) {
          var item, _j, _k, _len1, _len2;
          if (err) {
            return cb(err);
          }
          items || (items = []);
          ...
          for (_k = 0, _len2 = tasks.length; _k < _len2; _k++) {
            ...
            if (task.createdBy && task.createdBy.actorId) {
              actorId = task.createdBy.actorId.toString();
              task.username = _this.usernameMap[actorId] || '';
            }
          }
          return cb(null);
        });
    ...
    */
    ...
    RoutesApi.prototype.getAdminTaskForReopen = function(req, res, next) {
      var _this = this;
      return this.dbStore.tasks.get(req.params.taskId, {}, function(err, task) {
        if (err) {
          return next(err);
        }
        return _this._stateMachineForProcessDefinitionId(task.processDefinitionId, function(err, sm) {
          var obj;
          obj = task.toObject();
    ...
    RoutesApi.prototype.postAdminTaskForReopen = function(req, res, next) {
      var newState,
        _this = this;
      ...
      return this.dbStore.tasks.get(req.params.taskId, {}, function(err, task) {
        ...
        return _this._stateMachineForProcessDefinitionId(task.processDefinitionId, function(err, sm) {
          ...
          return _this.dbStore.tasks.patch(task._id, obj, {}, function(err) {
            if (err) {
              return next(err);
            }
            return res.json({});
