    ...
    */
    ...
    RoutesApi.prototype.createTask = function(req, res, next) {
      ...
      return this.dbStore.processDefinitions.get2(req.body.processDefinitionId, {
        select: '_id taskNamePrefix stateMachine'
      }, function(err, processDefinition) {
        if (err) {
          return next(err);
        }
        ...
        return stateMachineForProcessDefinition(processDefinition, function(err, sm) {
          if (err) {
            return next(err);
          }
          return _this.dbStore.processDefinitions.getNextTaskNumber(req.body.processDefinitionId, function(err, count) {
    ...
    */
    ...
    RoutesApi.prototype.getNextTask = function(req, res, next) {
      ...
      return this.dbStore.tasks.getActiveTask(req.user.id || req.user._id, {}, function(err, task) {
        ...
        return statesForRoles(req.user.roles, _this.dbStore, function(err, states) {
          if (err) {
            return next(err);
          }
          return _this.dbStore.tasks.getTaskForStates(states, {}, function(err, task) {
